...
const FoodItem = (item) => {
  ...
  return (
    <div className="food-item  rounded-xl flex flex-col items-center justify-between p-2 shadow group">
      <div className="food-image relative rounded-xl overflow-hidden transition duration-200">
        <img
          src={image}
          alt=""
          className="rounded-xl group-hover:scale-125 transition duration-300"
        />
            ...
            <div className="flex items-center gap-x-2 justify-center bg-white p-1 rounded-full">
              <button className="sub flex justify-center items-center">
                <img
                  ...
                  alt=""
                  className="w-7 cursor-pointer hover:scale-90 "
                  onClick={() => removeFromCart(_id)}
                />
              </button>
              <p className="count text-lg font-medium text-icon px-0.5">
                {cartItems[_id]}
              </p>
      ...
      <div className="food-info mt-3">
        <div className="price flex items-center text-icon gap-x-1 text-lg font-medium">
        ...
        <div className="name-rating flex justify-between items-center mb-1">
          <p className="name text-lg font-medium">{name}</p>
          <img src={assets.rating_starts} alt="" className="w-16" />
        </div>
        <p className="food-desc text-sm text-gray-600">{description}</p>
      </div>
    </div>
  );
};
